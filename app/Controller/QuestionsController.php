...
class QuestionsController extends AppController {
	...
	public function index($id = null, $contest_id = null) {
		...
		if($this->request->data) {
			...
			if($problem['Problem']['user_id'] == $this->Auth->user('id')) {
				$unanswered = $this->Question->find('all', array('conditions' => array('AND' => array('Question.problem_id' => $id, 'Question.public' => 2)),  'order' => 'Question.created DESC'));
			} else {
				$unanswered = $this->Question->find('all', array('conditions' => array('AND' => array('Question.problem_id' => $id, 'AND' => array('Question.public' => 2, 'Question.user_id' => $this->Auth->user('id')))),  'order' => 'Question.created DESC'));
			}
			...
			if($this->Auth->user('id')) {
				...
				$questions = $this->Question->find('all', array('conditions' => array('AND' => array('Question.problem_id' => $id, 'Question.public' => 0)),  'order' => 'Question.created DESC'));
			}
